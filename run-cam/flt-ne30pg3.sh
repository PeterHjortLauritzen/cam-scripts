...
echo "[flt-ne30pg3] Wrote user_nl_cam with requested settings for RUN_PROFILE=${RUN_PROFILE}."
...
if [[ "${PERF_ANALYSIS}" == "true" ]]; then
  if [[ -f user_nl_cam ]]; then
    ...
    echo "[flt-ne30pg3] --performance_analysis: removed fincl* lines from user_nl_cam."
  fi
fi
...
if [[ "${SKIP_BUILD}" == "true" ]]; then
  echo "[flt-ne30pg3] --skip-build set: skipping case.build and case.submit."
  echo "[flt-ne30pg3] Next steps:"
  echo "  cd ${CASEDIR}"
  echo "  ./case.build"
  echo "  ./case.submit"
  exit 0
...
echo "[flt-ne30pg3] Case ${CASE_NAME} submitted successfully."
